    ...
    * */
    ...
    private fun snap() {
        ...
        while (maleMap.isNotEmpty() && femaleMap.isNotEmpty()) {
            ...
            println("{\"x\":$populationLifeSpan,\"y\":${(femaleMap.keys.size + maleMap.keys.size)}},")

            if (populationLifeSpan == 800) {
                println("at 600")
                //println("Snap before population = ${(femaleMap.keys.size + maleMap.keys.size)}")
                thanosSnap()
                //println("Snap after population = ${(femaleMap.keys.size + maleMap.keys.size)}")
            }
    ...
    private fun thanosSnap() {

        val fLiveCount = femaleMap.keys.size
        val mLiveCount = maleMap.keys.size

        while (fLiveCount / 2 < femaleMap.keys.size) {

            //println("fCount = ${femaleMap.keys.size}")

            val fKeys = femaleMap.keys.toSet()

            fKeys.forEach {

                if (fLiveCount / 2 < femaleMap.keys.size && Random.nextInt(1,100)%2 == 0) {
                    femaleMap.remove(it)
                }
        ...
        while (mLiveCount / 2 < maleMap.keys.size) {

            //println("mCount = ${maleMap.keys.size}")

            val mKeys = maleMap.keys.toSet()

            mKeys.forEach {

                if (mLiveCount / 2 < maleMap.keys.size && Random.nextInt(1,100)%2 == 0) {
                    maleMap.remove(it)
                }
